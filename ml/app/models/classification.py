...
class MLPHead(nn.Module):
    def __init__(self, in_features: int, num_output_classes: int) -> None:
        super().__init__()
        self.classifier = nn.Sequential(nn.Linear(in_features, 2048), nn.GELU(), nn.Dropout(p=0.5),
...
class PretrainedModelFactory:
    ...
    def _efficientnet_b0():
        ...
        model.classifier = MLPHead(in_features=model.classifier[1].in_features,
    ...
    def __init__(self):
        self.available_models = {"efficientnet_b0": self._efficientnet_b0, "resnet_50": self._resnet_50,
                                 "vit_b_16": self._vit_b_16, "mobilenet_v3_small": self._mobilenet_v3_small, }
